...
def wordmodel(max_len):
    ...
    wordmodel = TimeDistributed(Dense(250))(model)  # softmax output layer
    print (wordmodel)
    plot_model(Model(input,wordmodel), show_shapes=True, to_file=outFileName+'wordModel-plot.png')
...
def charmodel(maxcharlen):
    ...
    model = Dropout(0.1)(model)
    model = Bidirectional(LSTM(units=100, return_sequences=True, recurrent_dropout=0.1))(model)
    charmodel = TimeDistributed(Dense(250))(model)  # softmax output layer
    #charmodel=Flatten()(charmodel)
    plot_model(Model(input,charmodel), show_shapes=True, to_file=outFileName+'characterModel-plot.png')
...
def define_model():
    ...
    out = Dense(num_classes, activation="softmax")(model)  # softmax output layer
...
hist = model.fit([trainseq, train_charseq], out_seq, epochs=15, batch_size=8,verbose=1, callbacks=callbacks_list,validation_split=0.1)
